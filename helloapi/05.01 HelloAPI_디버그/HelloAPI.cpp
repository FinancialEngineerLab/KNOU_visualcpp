					 ...
					 int nCmdShow)
{
	...
	OutputDebugString(L"CreateWindow - 메인 윈도우의 생성시작  \n");
	hWnd=CreateWindow(lpszClass,			//윈도우클래스 이름
					...
					hInstance,							//인스턴스핸들
					NULL);								//여분의 데이터
	 OutputDebugString(L"메인 윈도우의 생성  완료  \n");

 	   ShowWindow(hWnd,nCmdShow);
	OutputDebugString(L"메인 윈도우의 Show 완료  \n");
						 ...
						 WPARAM wParam, LPARAM lParam)
{
	...
	switch(iMessage) {
		case WM_PAINT:
			...
			OutputDebugString(L"(메인 윈도우)WndProc의  WM_PAINT 발생 처리\n");
				EndPaint(hWnd,&ps);
				return 0;
			}
		case WM_CREATE:
		{
        	 OutputDebugString(L"WndProc에서  child 윈도우 생성 시작  \n");
    	   ...
    	   OutputDebugString(L"WndProc 에서 child 윈도우 생성과 Show 완료 \n");	
			OutputDebugString(L"WndProc 에서 버튼 윈도우 의 생성 시작 \n");
			...
			if (!hChildWnd) 	return -1;
				OutputDebugString(L"WndProc 에서 버튼윈도 생성 및 Show 완료 \n");
			return 0;
		}
		case WM_COMMAND:
		{
			...
			OutputDebugString(L" MessageBox [확인]  처리 완료   \n");	
			return 0;
		}
		case WM_DESTROY:
			  OutputDebugString(L"WndProc 에서 WM_DESTROY 처리 시작   \n");	
			PostQuitMessage(0);
			  OutputDebugString(L"WndProc 에서 WM_DESTROY 처리 완료   \n");	
			return 0;
		}
...
LRESULT CALLBACK ChildWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_PAINT:
		{
			...
			EndPaint(hWnd,&ps);
			OutputDebugString(L"ChildWndProc에서  WM_PAINT 처리 완료 \n");
			return 0;
		}
